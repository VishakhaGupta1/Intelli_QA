...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/qa_platform';
...
function pickStatus(index) {
  ...
  if (bucket < 16) return 'PASS';
  if (bucket < 19) return 'FAIL';
  return 'SKIP';
}
...
async function main() {
  ...
  const testResults = Array.from({ length: 50 }, (_, index) => {
    ...
    return {
      ...
      endpoint: endpoints[index % endpoints.length].path,
      layer: index % 5 === 0 ? 'ui' : 'api',
      status,
      duration: randomInt(200, 3000),
      failureMessage: status === 'FAIL' ? 'Synthetic failure generated for dashboard seeding.' : null,
      failureType: status === 'FAIL' ? 'assertion' : null,
  ...
  const gapReports = [
    { endpoint: '/api/orders/1', method: 'GET', reason: 'No dedicated read-order regression yet.' },
    { endpoint: '/api/products/99999', method: 'GET', reason: 'Negative-path coverage is still incomplete.' },
    { endpoint: '/api/users/summary', method: 'GET', reason: 'Summary endpoint remains untested in the suite.' },
  ];
